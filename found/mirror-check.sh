#!/bin/bash
#
#  Chakra Mirror-Check - Version 1.91
...
# Downloads the given file to the given path or outputs error if it
# can't.
download_or_error() {
    wget "$1" -O "$2" &> /dev/null
    if [ "$?" != "0" ]; then
...
cli_results() {
    ...
    for unsynced in $2; do
	      echo -e "\e[01;37m[$unsynced]\e[00m \e[00;31mis not synced\e[00m."
    done
}
# No-op CLI version of gui_close_window
cli_close_window() { true; }
...
Exit code: 0 if all mirrors have been varified, otherwise 1
"
    echo "$_usage"
    exit 1
}
...
main() {
    case "$_mode" in
        --help) usage ;;
        --gui)  runit gui_error gui_message gui_results gui_close_window ;;
        --cli)  runit cli_error cli_message cli_results cli_close_window ;;
        *) cli_error "Unknown flag $_mode."
           usage ;;
    esac
}
